...
echo addNPCBox(36, $txt['casino_store_npc_title'], $txt['casino_store_npc_text']);
...
if (isset($_POST['buy-tickets-quant']) && ctype_digit(($_POST['buy-tickets-quant'] ?? ''))) {
    ...
    if ($gebruiker['silver'] <= $price) {
        echo '<div class="red">'.$txt['common_not_enough_silver'].'</div>';
    } else {
        DB::exQuery("UPDATE `gebruikers` SET `tickets`=`tickets`+'$tickets', `silver`=`silver`-'$price' WHERE `user_id`='$_SESSION[id]'");
        echo '<div class="green">'.sprintf($txt['casino_store_bought_tickets'], $tickets).'</div>';
    }
}
...
if (isset($_POST['sell-tickets-quant']) && ctype_digit(($_POST['sell-tickets-quant'] ?? ''))) {
    ...
    if ($gebruiker['tickets'] <= $price) {
        echo '<div class="red">'.$txt['common_not_enough_tickets'].'</div>';
    } else {
        DB::exQuery("UPDATE `gebruikers` SET `tickets`=`tickets`-'$price', `silver`=`silver`+'$silvers' WHERE `user_id`='$_SESSION[id]'");
        echo '<div class="green">'.sprintf($txt['casino_store_bought_silvers'], $silvers).'</div>';
    }
}
        ...
        if ($gebruiker['tickets'] <= $verify['price']) {
            ...
            if ($verify['type'] == 0) {
                if ($gebruiker['in_hand'] >= 6) {
                    echo '<div class="red">'.$txt['common_six_pokemon_in_hand'].'</div>';
                } else {
                ...
                DB::exQuery("UPDATE `pokemon_speler` SET `level`='5',`karakter`='".$karakter['karakter_naam']."',`expnodig`='".$experience['punten']."',`user_id`='".($_SESSION['id'] ?? '')."',`opzak`='ja',`opzak_nummer`='".$opzak_nummer."',`attack_iv`='".$attack_iv."',`defence_iv`='".$defence_iv."',`speed_iv`='".$speed_iv."',`spc.attack_iv`='".$spcattack_iv."',`spc.defence_iv`='".$spcdefence_iv."',`hp_iv`='".$hp_iv."',`attack`='".$attackstat."',`defence`='".$defencestat."',`speed`='".$speedstat."',`spc.attack`='".$spcattackstat."',`spc.defence`='".$spcdefencestat."',`levenmax`='".$hpstat."',`leven`='".$hpstat."',`ability`='".$ability."',`capture_date`='".$date."',`icon`='1' WHERE `id`='".$pokeid."' LIMIT 1");
                    ...
                    echo '<div class="green">'.sprintf($txt['common_bought_one_item'], $verify['name']).'</div>';
                }
            } else {
                if ($gebruiker['item_over'] < 1) {
                    echo '<div class="red">'.$txt['common_no_bag_space'].'</div>';
                } else {
...
<div class="box-content" style="margin-bottom: 7px"><h3 class="title" style="background: none"> <?=$txt['casino_store_inventory_tickets']?> <img src="<?=$static_url?>/images/icons/ticket.png" title="Tickets" />  <?= highamount($gebruiker['tickets']); ?></h3> </div>

<div class="box-content" style="margin-bottom: 7px">
    <table class="general" width="100%">
        <thead>
            <tr><th><?=$txt['casino_store_buy_or_exchange']?></th></tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    <center>
                        <div class="greyborder">
                            <table style="width: 125px; height: 101px;">
                                ...
                                <tr><td align="center"><span id="buy-silvers">50</span>x Tickets <span style="cursor:pointer;" title="<?=$txt['casino_store_silver_to_tickets']?>"><b>[?]</b></span></td></tr>
                        ...
                        <div class="greyborder">
                            <table style="width: 125px; height: 101px;">
                                ...
                                <tr><td align="center"><span id="sell-silvers">1.250</span> Silvers <span style="cursor:pointer;" title="<?=$txt['casino_store_tickets_to_silver']?>"><b>[?]</b></span></td></tr>
...
<div class="box-content">
    <table class="general" width="100%">
        <thead>
            <tr><th><?=$txt['casino_store_rewards_title']?></th></tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    <center>
                        <?php
                            while ($st = $store->fetch_assoc()) {
                                if ($st['type'] == 0) {
                                    ...
                                    $desc = sprintf($txt['casino_store_desc_pokemon'], $st['name']);
                                } else {
                                    ...
                                    $desc = sprintf($txt['casino_store_desc_item'], $st['name'], $select['omschrijving_pt']);
                                }
                        ?>
                        ...
                        <div class="greyborder">
                            <table style="width: 125px; height: 101px;">
                                ...
                                <tr><td align="center"><form method="post" onsubmit="return confirm('<?=$txt['common_confirm_purchase']?>')"><input type="hidden" name="buy" value="<?=$st['id']?>"><input type="submit" value="<?=$txt['common_buy']?>" name="buy-casino"></form></td></tr>
                            </tbody></table>
                        </div>
