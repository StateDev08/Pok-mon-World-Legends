...
3. Follow the assistant: it creates your `.env` file, connects to the database and imports the bundled schema.
...
The `.env` file is loaded by `app/includes/Env.php`. The schema also ships a number of in-code feature toggles (e.g. weekend triple XP, double silver days, seasonal shops) in `app/includes/resources/config.php`.
